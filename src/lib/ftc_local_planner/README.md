...
Sharp turn vertices are identified when a plan is accepted using `sharp_corner_angle`, `turnaround_angle_threshold`, and `turnaround_preview_distance`; changes to those parameters affect the next plan. Intermediate stop vertices and the final pose use the same latency-compensated braking profile. The controller keeps steering on the incoming line while braking, settles measured linear motion, and accepts the target only within its full two-dimensional position tolerance.

If the mower settles outside a stop tolerance with significant lateral error, it can reverse straight by `vertex_reapproach_distance`, rotate toward the missed vertex, and track a temporary straight line back to it without modifying the supplied plan. `vertex_reapproach_attempts` limits these retries. An unsafe or exhausted optional re-approach returns MBF `MISSED_GOAL`; collision on the commanded path still returns `COLLISION`.
