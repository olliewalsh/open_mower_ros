...
ros::ServiceClient pathClient, mapClient, dockingPointClient, gpsClient, calibrateGyroClient, mowClient,
...
bool setMowerEnabled(bool enabled) {
  ...
  if (last_status.mow_enabled != enabled) {
    ...
    mow_srv.request.mow_direction = (started.sec / 3600) & 0x1;  // Randomize mower direction on hour
