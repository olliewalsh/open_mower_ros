...
void publishStatus(const ros::TimerEvent &timer_event) {
  ...
  is_docked =
      has_dock && sqrt(pow(pose.pose.pose.position.x - dockX, 2) + pow(pose.pose.pose.position.y - dockY, 2)) < 0.25;
...
int main(int argc, char **argv) {
  ...
  */
  ...
  fake_mow_status.stamp = ros::Time::now();
  fake_mow_status.left_esc_status.status = fake_mow_status.right_esc_status.status =
      fake_mow_status.mow_esc_status.status = mower_msgs::ESCStatus::ESC_STATUS_OK;
