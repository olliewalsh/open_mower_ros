...
void TestConstantSpeedWithQuantizedTicks() {
  constexpr float kTicksPerMeter = 1195.0f;
  constexpr float kSpeed = 0.35f;
  const float resolution = 1.0f / kTicksPerMeter;
  ...
  for (int index = 0; index < 4000; ++index) {
    ...
    position += kSpeed * dt;
    const int32_t ticks = QuantizedTicks(position, kTicksPerMeter);
    ...
    if (index > 100) {
      const double observer_error = observer.velocity() - kSpeed;
      const double window_speed = window_tick_sum / (window_dt_sum * kTicksPerMeter);
      const double window_error = window_speed - kSpeed;
...
void TestAccelerationAndStopResponse() {
  constexpr float kTicksPerMeter = 1195.0f;
...
void TestPureRotationCancellation() {
  constexpr float kTicksPerMeter = 1195.0f;
  ...
  for (int index = 0; index < 500; ++index) {
    ...
    const int32_t left_ticks = QuantizedTicks(left_position, kTicksPerMeter);
    const int32_t right_ticks = QuantizedTicks(right_position, kTicksPerMeter);
...
int main() {
  TestConstantSpeedWithQuantizedTicks();
  TestAccelerationAndStopResponse();
  TestPureRotationCancellation();
